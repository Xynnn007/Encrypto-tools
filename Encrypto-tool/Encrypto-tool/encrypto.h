...
void DES::encrypt(const unsigned char* plaintext, unsigned char* cipher)
{
	...
	unsigned char R_extern[48]; //扩展R结果
	unsigned char R_s[32];		//S替换结果
	unsigned char R_p[32];		//P置换结果
	for (int i = 7; i >= 0; i--)
	{
		for (int j = 7; j >= 0; j--)
		{
	...
	//获得明文的每个位上的数据数组 plain_array[64]
	//初次置换，获得new_array[64]
	for (int i = 0; i < 64; i++)
	{
		new_array[i] = plain_array[IP[i] - 1];
	}
	//把初次置换后的结果进行拆分，形成L R两部分，分别是64位的前32位和后32位
	for (int i = 0; i < 32; i++)
	{
		L0[i] = new_array[i];
		R0[i] = new_array[i + 32];
	...
	for (int n = 0; n < 16; n++)
	{
		...
		//接下来做S盒代替，把48位扩展R分成8个6位分组，每个分组进一个盒子
	...
	//得到的L16和R16调换拼接，做P-1逆置换，存到new_array[64]里
	for (int i = 0; i < 32; i++)
	{
		new_array[i + 32] = L1[i];
		new_array[i] = R1[i];
...
void DES::setkey(const unsigned char* key_ori)
{
	...
	//根据PC_1L和PC_1R置换，获得C0[28]和D0[28]
	for (int i = 0; i < 28; i++)
	{
		C0[i] = key[PC_1L[i] - 1];
		D0[i] = key[PC_1R[i] - 1 ];
